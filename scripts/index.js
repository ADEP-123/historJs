...
document.addEventListener("DOMContentLoaded", () => {
    ...
    variables.startButton.addEventListener("click", (e) => {
        ...
        if (minUser.value == "" || secUser.value == "") {
            ...
            if (parseInt(minUser.value) > 59 || parseInt(minUser.value) < 0) {
                alert("Los minutos no pueden ser mayores a 59 ni menor a 0")
            } else if (parseInt(secUser.value) > 59 || parseInt(secUser.value) < 0) {
                alert("Los segundos no pueden ser mayores a 59 ni menor a 0")
            } else {
                if (!minUser.value[1]) {
